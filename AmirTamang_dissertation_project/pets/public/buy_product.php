...
if (isset($_POST['add_to_cart'])) {
    $item_id = $_POST['item_id']; //get the item id from the product table
    ...
    // redirect back to store.php or wherever you want
...
try {
    ...
    die("Error: " . $e->getMessage()); // handle exceptions and displays errors
}
...
if (isset($_POST['search'])) {
    ...
    $filteredProducts = array_filter($products, function ($product) use ($searchQuery) {
        return stristr($product['product_name'], $searchQuery) !== false;
    });
    if (empty($filteredProducts)) {
        $noResultsMessage = "No results found."; // display a message if no result is found
    }
} else {
...
<body>
    ...
    <div class="container-fluid py-5">
        <div class="container">
            ...
            <li><a href="">Product for pet</a></li>
        </div>
    </div>
    ...
    <div class="container-fluid py-5">
        <div class="container">
            ...
            <div class="row">
                ...
                <?php else : ?>
                    <!-- foreach loop for filtered products -->
                    <?php foreach ($filteredProducts as $product) : ?>
                        <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
                            <div class="product-item position-relative bg-light d-flex flex-column text-center">
                                <div>
                                    <!-- display image -->
                                    <img src="<?= $product['product_image']; ?>" alt="<?= $product['product_name']; ?>" class="img-fluid" style="max-height: 120px">
                                </div>
                                <!-- display informations -->
                                <h6 class="text-uppercase"><?= $product['product_name']; ?></h6>
                                <h5 class="text-primary mb-0">Rs.<?= $product['retail_price']; ?></h5>
                                <div class="btn-action d-flex justify-content-center">
                                    <!-- hidden form for items information -->
                                    <form action="buy_product.php" method="post">
                                        ...
                                        <input type="hidden" name="item_price" value="<?= $product['retail_price']; ?>">
